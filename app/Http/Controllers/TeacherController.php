    ...
    public function login()
    {
        ...
        public function forgot_password_post(Request $request) {
            ...
            if (count($teacher) == 1) {
                ...
                $action_link = route('teacher.reset.password', ['token' => $token, 'email' => $request->email]);
                $body = 'We Received a Request to Reset the Password for <strong>Quizapp Teacher Portal</strong> account associated with '.$request->email. '. You can Reset Your Password by Clicking the Link Below: ';
        ...
        public function reset_password($token, $email) {
            return view('teacher.resetPassword', ['token' => $token, 'email' => $email]);
        }
