...
class ParentController extends Controller
{
    ...
    public function forgot_password_post(Request $request) {
        $parent = Parents::where('email', $request->email)
                        ->get()
                        ->toArray();
        if (count($parent) == 1) {
            $token = \Str::random(64);
            ...
            $action_link = route('parent.reset.password', ['token' => $token, 'email' => $request->email]);
            $body = 'We Received a Request to Reset the Password for <strong>Quizapp Parent Portal</strong> account associated with '.$request->email. '. You can Reset Your Password by Clicking the Link Below: ';

            $data = [
                'action_link' => $action_link,
                'body' => $body
            ];
            \Mail::mailer('sendmail')->to($request->email)->send(new PasswordResetLink($data));

            $success = 'We have Mailed you the Reset Link. Please Check Your Email';
            session()->flash('success', $success);
        }
        else {
            $error = "Please Enter a Registered Email Address";
    ...
    public function reset_password($token, $email) {
        return view('parent.resetPassword', ['token' => $token, 'email' => $email]);
    }
    ...
    public function reset_password_post(Request $request) {
        ...
        if (!$check_token) {
            ...
            Parents::where('email', $request->email)
                ->update([
        ...
        $success = "We have Successfully Changed Your Password. Please Login with New Credentials";
        session()->flash('success', $success);
        return view('parent.login');
    }
    ...
    public function parent_addkidPost(Request $request)
    {
        ...
        try {
            $hex = ['#F8DF81', '#F6AA90', '#F6B4BF', '#D5B6D5', '#BADFDA', '#9BD0B7', '#E5EBD7', '#96C3EB', '#E6E6E6', '#FFE3D1'];
